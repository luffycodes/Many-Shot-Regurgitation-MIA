...
if __name__ == '__main__':
    ...
    while True:
        ...
        if args.gpt == 0:
            result_df.to_csv(f'{identifier}/{identifier}_{str(num_words)}_fixed_do_sample_0_splits_{str(round(args.temp * 10))}_splits_{str(n)}_{book_name}', index=False)
        else:
            result_df.to_csv(f'wiki/{folder_name}/gpt3_{str(num_words)}_fixed_temp_dot{str(round(args.temp*10))}_splits_{str(n)}_{book_name}', index=False)
